...
제출하고 답이 체크되는 동안 이런 성능은 어떻게 나올 수 있는지 정말 궁금해졌다. 이런 코드를 작성하는 사람은 나사에서 일하는 사람인가?
...
눈여겨 볼 점
1. 세트끼리의 bitwise and 연산 -> 그냥 교집합
    - 각 수에 대해 세트로 만들고 고장난 버튼과 교집합이 공집합이라면 현재 x와 현재 지점과 목표지점의 차이+현재지점까지 고장 안난 버튼 누른 횟수를 비교해 더 작은 값을 기록한다.
2. 범위: 목표 지점 n과 n과 100의 차이를 더한 수까지만 순회한다.
    - 왜 x+n까지인가?
        1. n이 500_000인 경우
            - 모든 버튼이 고장났을 때 답은 499_900이다. 즉 순회할 필요가 없다.
        2. n이 0인 경우
            - 
