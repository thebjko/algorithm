...
### 다른 코드 분석
[sait2000님의 코드](https://www.acmicpc.net/source/5517526):
```python
a=m=0;exec('a-=eval(input().replace(*" -"));m=max(m,a);'*4);print(m)

# Equivalently:
a = m = 0
exec('a -= eval(input().replace(*" -")); m = max(m, a);' * 4)
print(m)
```
> 메모리 약 29MB, 시간 56ms

1. `exec`는 리턴값이 없고 `eval`은 있다.
2. `" -"`를 언패킹하여 공백문자를 마이너스 기호로 바꾼다. 개행문자는 그대로 둔다.
3. 각 행은 `{숫자}-{숫자}(\n)`와 같이 되고 `eval`함수에 의해 계산된 값을 `a`에서 뺀다.
4. `m`과 `a`를 비교하여 큰 값을 `m`에 저장하고 4번 실행한다.
	- 예를 들어:
		- 첫 번째 입력값 0 32 -> 0-32 -> `a = 32` -> `m = 32`
		- 두 번째 입력값 3 13 -> 3-13 -> `a = 42` -> `m = 42`
		- 세 번째 입력값 28 25 -> 28-25 -> `a = 39` -> `m = 42`
		- 네 번째 입력값 39 0 -> 39-0 -> `a = 0` -> `m = 42`